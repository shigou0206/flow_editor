  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final edge in edges) {
      ...
      if (points != null && points.length >= 2) {
        ...
        final path = Path();
        path.moveTo(
            sourcePoint.dx + canvasOffset.dx, sourcePoint.dy + canvasOffset.dy);
        ...
        if (routePoints.length >= 3) {
          for (var i = 1; i < routePoints.length - 1; i++) {
            ...
            if ((dx1 != 0 && dx2 != 0) || (dy1 != 0 && dy2 != 0)) {
              ...
              final beforeCornerX = currentPoint.dx -
                  dx1 * cornerRadius / sqrt(dx1 * dx1 + dy1 * dy1);
              final beforeCornerY = currentPoint.dy -
                  dy1 * cornerRadius / sqrt(dx1 * dx1 + dy1 * dy1);

              // 计算转角后的点（距离转角cornerRadius的距离）
              final afterCornerX = currentPoint.dx +
                  dx2 * cornerRadius / sqrt(dx2 * dx2 + dy2 * dy2);
              final afterCornerY = currentPoint.dy +
                  dy2 * cornerRadius / sqrt(dx2 * dx2 + dy2 * dy2);

              // 先画直线到转角前的点
              path.lineTo(beforeCornerX + canvasOffset.dx,
              ...
              // 如果方向没有变化，则直接连接到当前点
              path.lineTo(currentPoint.dx + canvasOffset.dx,
                  currentPoint.dy + canvasOffset.dy);
            }
          }
          ...
          path.lineTo(
              lastPoint.dx + canvasOffset.dx, lastPoint.dy + canvasOffset.dy);
        } else {
          // 如果没有中间点，直接连接到最后一个点
          for (var i = 1; i < routePoints.length; i++) {
            path.lineTo(routePoints[i].dx + canvasOffset.dx,
        ...
        if (debug) {
          ...
          // 绘制目标节点中心点（蓝色）
          canvas.drawCircle(
              Offset(targetPoint.dx + canvasOffset.dx,
                  targetPoint.dy + canvasOffset.dy),
              4.0,
              Paint()
                ..color = Colors.blue
                ..style = PaintingStyle.fill);
          ...
          // 绘制倒数第二个点（红色）- 用于计算方向
          canvas.drawCircle(
              Offset(secondLastPoint.dx + canvasOffset.dx,
                  secondLastPoint.dy + canvasOffset.dy),
              4.0,
              Paint()
                ..color = Colors.red
                ..style = PaintingStyle.fill);
          ...
          textPainter.paint(
              canvas,
              Offset(midPoint.dx + canvasOffset.dx,
