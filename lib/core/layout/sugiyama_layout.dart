  ...
  void performLayout(
    ...
    void layoutGroup(String? parentId) {
      for (final group
        ...
        performGraphLayoutForGroup(group, nodes, edges);
      }
    }
...
void performLayout(List<NodeModel> nodes, List<EdgeModel> edges) {
  void layoutGroup(String? parentId) {
    for (final group
        in nodes.where((n) => n.parentId == parentId && n.isGroup)) {
      layoutGroup(group.id);
...
void performGraphLayoutForGroup(
  ...
  final groupEdges = edges.where((edge) {
    final src = nodes.firstWhereOrNull((node) => node.id == edge.sourceNodeId);
    final tgt = nodes.firstWhereOrNull((node) => node.id == edge.targetNodeId);
    return src != null &&
        tgt != null &&
